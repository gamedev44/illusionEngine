...
namespace illusion::graphics::d3d12::content {
namespace {
...
struct {
    utl::vector<illusion::content::lod_offset>    lod_offsets;
    ...
    { // Lock materials
        ...
        for (u32 i{ 0 }; i < shader_type::count; ++i)
        {
            if (flags & (1 << i))
            {
                ...
                illusion::content::compiled_shader_ptr shader{ illusion::content::get_shader(material.shader_ids()[shader_index], key) };
                assert(shader);
    ...
    u32 material_count, const id::id_type *const material_ids)
{
    ...
    illusion::content::get_submesh_gpu_ids(geometry_content_id, material_count, gpu_ids);
...
get_d3d12_render_item_ids(const frame_info& info, utl::vector<id::id_type>& d3d12_render_item_ids)
{
    ...
    illusion::content::get_lod_offsets(frame_cache.geometry_ids.data(), info.thresholds, count, frame_cache.lod_offsets);
    ...
    for (u32 i{ 0 }; i < count; ++i)
    {
        ...
        const illusion::content::lod_offset& lod_offset{ frame_cache.lod_offsets[i] };
